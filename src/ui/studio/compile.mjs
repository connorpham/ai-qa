...
export function compile(flow, ctx = {}) {
  ...
  for (const n of order) {
    ...
    switch (n.type) {
      ...
      case "expect":
        ...
        // `cite` travels with the check. It is the field that decides whether a
        // difference is a DEFECT, and it was being dropped here — so the image
        // that proves the check could never say where the rule is written.
        lastExpect = { what: x.what, selector: x.selector, value: x.value, cite: x.cite || "" };
        webPlan.push({ op: "expect", ...lastExpect });
        // No separate plain shot: the expect now photographs itself, boxed and
        // captioned. A second unboxed copy of the same moment is noise in a
        // folder someone has to read.
        break;
      case "screenshot":
  ...
  if (isWeb) {
    files[path.posix.join(caseDir, "journey.mjs")] = renderJourney(webPlan, { toRoot, appUrl, flowName: flow.name,
      // Burned into every annotated image, because a screenshot leaves its
      // folder almost immediately and out there it belongs to nothing.
      caseTitle: `${flow.ticket} · ${flow.name}` });
...
// renderers
// ---------------------------------------------------------------------------
function renderJourney(plan, { toRoot, appUrl, flowName, caseTitle = "" }) {
  ...
  const out = [
    ...
    `import { launch, click, typeIn, beat, shot, shotAnnotated, close } from ${J(toRoot + ".ai-qa/scripts/browser.mjs")};`,
    ``,
  ...
  for (const s of plan) {
    switch (s.op) {
      ...
      case "expect": {
        // The check now leaves a picture of itself. The selector the case
        // already declares becomes the box, and the caption writes itself from
        // the same fields — so nobody picks coordinates and nobody can box the
        // wrong element. The verdict colours it, so a red image is a red image
        // before anyone reads a word of it.
        out.push(
          `  {`,
          `    const actual = (await read(page, ${J(s.selector)})).trim();`,
          `    const ok = record(${J(s.what)}, ${J(s.value)}, actual);`,
          `    await shotAnnotated(page, HERE, ++n, ${J(s.what)}, {`,
          `      selector: ${J(s.selector)},`,
          `      title: ${J(caseTitle)} + " — step " + n,`,
          `      caption: ${J(s.what)} + ": must read " + ${J(s.value)} + " · actually read " + JSON.stringify(actual),`,
          `      cite: ${J(s.cite || "")},`,
